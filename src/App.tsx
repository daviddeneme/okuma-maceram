...
// ==========================================
// 1. PARÇANIN SONU - ALT SATIRA GEÇİP 2. PARÇAYI YAPIŞTIRIN
// ==========================================
return (
    <div className="min-h-screen bg-gradient-to-br from-sky-300 via-purple-200 to-fuchsia-200 font-sans flex flex-col relative pt-8 pb-12 overflow-x-hidden">
      ...
      {/* --- ÜST PANEL: ÖĞRENCİ PROFİLİ VE ATEŞ SERİSİ --- */}
                 ...
                 <div className="bg-fuchsia-50 p-4 rounded-2xl border-4 border-fuchsia-100 min-h-[100px] shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-3">
                       <div className="bg-white px-4 py-2 rounded-xl flex items-center gap-3 shadow-sm border-2 border-fuchsia-200">
                          <span className="text-2xl drop-shadow-md">🕵️‍♂️</span>
                          <span className="font-black text-fuchsia-700 text-xl">x {stats.filter(s => s.name === (savedProfile?.studentName || studentName) && s.badge).length}</span>
                       </div>
                       {teacherStars > 0 && (
      ...
      {/* 🎁 ÖĞRETMEN YILDIZ HEDİYE ANİMASYONU (GERİ GELDİ) */}
      {showTeacherStarGift && (
      ...
      <div className="flex-1 w-full px-4">
        ...
        {view === 'student-setup' && !isGeneratingStory && (
          <div className="max-w-xl mx-auto bg-white/95 p-8 rounded-[3rem] shadow-2xl border-8 border-sky-300 mt-24 relative text-center">
             ...
             <div className="space-y-6">
                ...
                <div className="space-y-4">
                   <button onClick={handleStartFreeReading} className="w-full bg-sky-500 text-white py-6 rounded-2xl text-2xl font-black border-b-8 border-sky-700 shadow-xl active:translate-y-2 active:border-b-0 transition-all">HİKAYEMİ OLUŞTUR ✨</button>
                   <button onClick={() => { if (!validateStudent()) return; setLoginError(''); setView('academy-menu'); }} className="w-full bg-indigo-500 text-white py-4 rounded-2xl text-xl font-black border-b-8 border-indigo-700 shadow-xl flex items-center justify-center gap-3 active:translate-y-2 active:border-b-0 transition-all">
        ...
        {view === 'academy-menu' && (
          <div className="max-w-4xl mx-auto bg-white/95 p-10 rounded-[3rem] shadow-2xl border-8 border-indigo-300 mt-20 text-center relative">
             ...
             <div className="grid md:grid-cols-2 gap-6">
                ...
                <button onClick={() => academyLevel >= 2 ? startSchulte() : null} className={`p-8 border-4 rounded-[2rem] text-left relative overflow-hidden transition-all shadow-sm ${academyLevel >= 2 ? 'bg-emerald-50 border-emerald-200 hover:bg-emerald-100 cursor-pointer group' : 'bg-slate-50 border-slate-200 opacity-60 cursor-not-allowed'}`}>
                ...
                <button onClick={() => academyLevel >= 3 ? startFlash() : null} className={`p-8 border-4 rounded-[2rem] text-left relative overflow-hidden transition-all shadow-sm ${academyLevel >= 3 ? 'bg-amber-50 border-amber-200 hover:bg-amber-100 cursor-pointer group' : 'bg-slate-50 border-slate-200 opacity-60 cursor-not-allowed'}`}>
                ...
                <button onClick={() => academyLevel >= 4 ? startMetronome() : null} className={`p-8 border-4 rounded-[2rem] text-left relative overflow-hidden transition-all shadow-sm ${academyLevel >= 4 ? 'bg-fuchsia-50 border-fuchsia-200 hover:bg-fuchsia-100 cursor-pointer group' : 'bg-slate-50 border-slate-200 opacity-60 cursor-not-allowed'}`}>
        ...
        {view === 'teacher' && (
          <div className="max-w-6xl mx-auto bg-white/95 rounded-[3rem] shadow-2xl border-8 border-emerald-100 mt-12 min-h-[600px] p-8 md:p-12 relative animate-in fade-in duration-300">
            ...
            {/* 📊 RADAR SEKMESİ VE SINIF ÖZETİ (YENİDEN EKLENDİ) */}
            {teacherTab === 'radar' && (
