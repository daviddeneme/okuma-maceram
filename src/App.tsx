          ...
          <div className="flex-1 flex flex-col items-center justify-center w-full z-20 pb-32 px-4">
            ...
            {!isAuthenticated ? (
              ...
              <div className="flex flex-wrap items-center justify-center gap-8 md:gap-12 w-full max-w-6xl animate-in slide-in-from-bottom-10 duration-500">
                ...
                <div className="w-[300px] md:w-[360px] bg-gradient-to-b from-amber-300 to-orange-400 border-8 border-orange-500 rounded-[2.5rem] p-6 shadow-2xl hover:-translate-y-2 transition-transform flex flex-col">
          ...
          <div className="flex-1 w-full px-4 max-w-6xl mx-auto">
            ...
            {view === 'reading-active' && (
              <div className="max-w-5xl mx-auto mt-12 space-y-8 relative">
                ...
                {/* OKUMA METNİ */}
                <div className="bg-white p-8 md:p-14 rounded-[3rem] shadow-2xl border-8 border-sky-200 relative mt-8">
                  <div className="absolute -top-6 right-8 z-10">
                    <button onClick={() => setShowHeceler(!showHeceler)}
                      className={`px-6 py-3 rounded-full font-black flex items-center gap-2 transition-all duration-300 shadow-xl border-4 border-fuchsia-400 text-lg ${showHeceler ? 'bg-fuchsia-500 text-white shadow-[0_0_20px_rgba(236,72,153,0.8)]' : 'bg-fuchsia-50 text-fuchsia-600 hover:bg-fuchsia-100 animate-pulse'}`}>
                      <Sparkles size={24} /> HECEMATİK {showHeceler ? 'AÇIK' : 'KAPALI'}
                    </button>
                  </div>
                  <p className="text-3xl md:text-[2.5rem] leading-[1.8] md:leading-[4.5rem] font-bold text-slate-800 whitespace-pre-wrap mt-4 cursor-default tracking-wide">
                    {renderStoryText()}
                  </p>
                </div>
            ...
            {view === 'result' && readingResult && (
              <div className="max-w-3xl mx-auto bg-white/95 p-12 rounded-[4rem] shadow-2xl border-8 border-sky-300 mt-12 text-center space-y-10 animate-in slide-in-from-bottom-10">
                ...
                <div className="bg-indigo-50 p-10 rounded-3xl font-bold text-indigo-900 text-2xl relative shadow-inner border-4 border-indigo-200 leading-relaxed text-left italic">
                  ...
                  <button onClick={() => speakInstruction(readingResult.aiEvaluation.geribildirim)} className="absolute -top-8 -right-8 bg-amber-400 text-amber-900 p-5 rounded-full shadow-xl hover:bg-amber-300 transition-transform active:scale-95 animate-bounce border-4 border-white" title="Sesli Dinle">
                    <Volume2 size={36} />
                  </button>
                </div>
              ...
              <div className="max-w-7xl mx-auto bg-white/95 rounded-[3rem] shadow-2xl border-8 border-emerald-200 mt-16 min-h-[600px] p-8 md:p-14 relative animate-in fade-in duration-300">
                ...
                {teacherTab === 'stats' && (
                  <div id="print-section" className="animate-in fade-in duration-300">
                    <div className="mb-10 flex flex-col md:flex-row items-center gap-6 bg-emerald-50 p-8 rounded-[2.5rem] border-4 border-emerald-200 no-print shadow-sm">
                      <label className="font-black text-emerald-900 text-2xl whitespace-nowrap">Öğrenci Seç:</label>
                      <select value={selectedStudentForProgress || ''} onChange={(e) => setSelectedStudentForProgress(e.target.value || null)} className="flex-1 w-full p-5 border-4 border-emerald-300 rounded-2xl font-black text-emerald-900 bg-white outline-none text-xl cursor-pointer">
                        <option value="">Tüm Sınıf (Genel Liste)</option>
                        {students.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}
                      </select>
                      {selectedStudentForProgress && (
                        <button onClick={() => window.print()} className="w-full md:w-auto bg-sky-500 hover:bg-sky-400 text-white p-5 rounded-2xl shadow-md font-black flex items-center justify-center gap-3 text-xl transition-colors border-b-4 border-sky-700 active:border-b-0 active:translate-y-1"><Printer size={28} /> Veli PDF Çıktısı</button>
                      )}
                    </div>

                    {selectedStudentForProgress && (
                      <div className="bg-white p-10 rounded-[3rem] border-4 border-emerald-100 mb-10 shadow-sm overflow-x-auto">
                        <h4 className="font-black text-emerald-900 mb-10 text-center text-3xl">{selectedStudentForProgress} - WPM Gelişim Grafiği</h4>
                        {(() => {
                          const studentStats = [...stats].filter(r => r.name === selectedStudentForProgress).reverse();
                          if (studentStats.length === 0) return <div className="text-center text-emerald-600 font-bold text-xl bg-emerald-50 p-6 rounded-2xl">Grafik oluşturulacak veri yok.</div>;
                          const allWpm = studentStats.map(s => Number(s.wpm) || 0);
                          const maxWpm = Math.max(...allWpm, 50) + 10;
                          const chartHeight = 250; const pointWidth = 100;
                          ...
                          return (
                            <div className="w-full overflow-x-auto pb-8">
                              <svg width={chartWidth} height={chartHeight + 60} className="mx-auto block">
                                <line x1="0" y1={chartHeight} x2={chartWidth} y2={chartHeight} stroke="#cbd5e1" strokeWidth="3" strokeDasharray="6,6" />
                                <polyline points={points} fill="none" stroke="#10b981" strokeWidth="6" strokeLinejoin="round" strokeLinecap="round" />
                                {studentStats.map((row, idx) => { const x = idx * pointWidth + 60; const y = chartHeight - ((Number(row.wpm) || 0) / maxWpm) * chartHeight; return (<g key={idx}><circle cx={x} cy={y} r="8" fill="#10b981" stroke="#fff" strokeWidth="3" className="hover:r-10 transition-all cursor-pointer"/><text x={x} y={y - 20} textAnchor="middle" className="text-[16px] font-black fill-emerald-800">{row.wpm}</text><text x={x} y={chartHeight + 30} textAnchor="middle" className="text-[12px] font-bold fill-slate-500">{row.date?.split(' - ')[0]}</text></g>); })}
                              </svg>
                            </div>
                          );
                        })()}
                    ...
                    <div className="overflow-x-auto bg-white rounded-[3rem] border-4 border-emerald-100 shadow-sm p-4">
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b-4 border-emerald-200 bg-emerald-50 rounded-t-2xl">
                            {!selectedStudentForProgress && <th className="p-6 font-black text-emerald-900 text-lg rounded-tl-2xl">İsim</th>}
                            <th className="p-6 font-black text-emerald-900 text-lg">Tarih - Saat</th>
                            <th className="p-6 font-black text-emerald-900 text-lg">Tür</th>
                            <th className="p-6 font-black text-emerald-900 text-center text-lg">Hız (WPM)</th>
                            <th className="p-6 font-black text-emerald-900 text-center text-lg">Skor</th>
                            <th className="p-6 font-black text-emerald-900 text-center text-lg no-print">Ses Kaydı</th>
                            <th className="p-6 font-black text-emerald-900 text-center text-lg no-print rounded-tr-2xl">İşlem</th>
                          </tr>
                        </thead>
                        <tbody>
                          {(selectedStudentForProgress ? stats.filter(r => r.name === selectedStudentForProgress) : stats).map(row => (
                            <tr key={row.id} className="border-b-2 border-slate-100 font-bold text-slate-700 hover:bg-emerald-50/50 transition-colors">
                              {!selectedStudentForProgress && <td className="p-6 text-emerald-900 text-xl font-black">{row.name}</td>}
                              <td className="p-6 text-md text-slate-500">{row.date || 'Belirtilmedi'}</td>
                              <td className="p-6">{row.level === 'Ödev' ? <span className="bg-amber-100 text-amber-800 px-4 py-2 rounded-xl text-sm border-2 border-amber-300 font-black">Ödev</span> : <span className="bg-sky-100 text-sky-800 px-4 py-2 rounded-xl text-sm border-2 border-sky-300 font-black">{row.interest}</span>}</td>
                              ...
                              <td className="p-6 text-center no-print">{row.audioUrl ? <audio src={row.audioUrl} controls className="h-12 w-full max-w-[240px] mx-auto outline-none" /> : <span className="text-slate-400 text-md bg-slate-100 px-4 py-2 rounded-xl">Yok</span>}</td>
                              <td className="p-6 text-center no-print"><button onClick={() => handleDeleteStat(row.id)} className="bg-rose-100 text-rose-600 p-4 rounded-xl hover:bg-rose-500 hover:text-white transition-colors border-2 border-rose-200"><Trash2 size={24} /></button></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                {teacherTab === 'homework' && (
                  <div className="space-y-10 animate-in fade-in duration-300">
                    {activeHomework && (
                      <div className="bg-amber-50 p-10 rounded-[3rem] border-4 border-amber-300 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
                        <div className="absolute -left-6 -top-6 text-amber-200/50 text-9xl pointer-events-none">📚</div>
                        <div className="relative z-10">
                          <h3 className="text-3xl font-black text-amber-900 mb-2">Şu An Yayında Aktif Ödev Var!</h3>
                          <p className="font-bold text-amber-700 text-xl flex items-center gap-2"><Clock/> Bitiş Süresi: {activeHomework.deadline ? new Date(activeHomework.deadline).toLocaleString('tr-TR') : 'Süresiz'}</p>
                        </div>
                        <button onClick={handleRemoveHomework} className="w-full md:w-auto bg-rose-500 hover:bg-rose-600 text-white px-8 py-5 rounded-2xl font-black shadow-[0_6px_0_rgb(190,18,60)] active:translate-y-2 active:shadow-none flex items-center justify-center gap-3 text-xl transition-all relative z-10"><Trash2 size={28} /> Aktif Ödevi Kaldır</button>
                      </div>
                    )}
                    <div className="bg-fuchsia-50 p-10 rounded-[3rem] border-4 border-fuchsia-200 flex flex-col md:flex-row gap-6 items-center shadow-sm">
                      <h3 className="w-full text-3xl font-black text-fuchsia-900 mb-2 flex items-center gap-3"><Sparkles className="text-fuchsia-500" size={36} /> YZ ile Ödev Üret</h3>
                      <input type="text" placeholder="Ödev Konusu (Örn: Orman Dostları)" value={hwTopic} onChange={e => setHwTopic(e.target.value)} className="w-full md:flex-1 p-5 border-4 border-fuchsia-300 rounded-2xl font-black outline-none text-xl text-fuchsia-900 focus:border-fuchsia-500 transition-colors placeholder-fuchsia-300" />
                      <select value={hwLevel} onChange={e => setHwLevel(e.target.value)} className="w-full md:w-auto p-5 border-4 border-fuchsia-300 rounded-2xl font-black bg-white text-fuchsia-900 text-xl outline-none cursor-pointer focus:border-fuchsia-500 transition-colors">
                        <option value="1">🌱 Kolay</option><option value="2">📚 Orta</option><option value="3">🔥 Zor</option>
                      </select>
                      <button onClick={handleGenerateHomeworkAI} disabled={isGeneratingHw} className="w-full md:w-auto bg-fuchsia-500 hover:bg-fuchsia-400 text-white font-black px-10 py-5 rounded-2xl flex items-center justify-center gap-3 transition-all text-xl shadow-[0_6px_0_rgb(162,28,175)] active:translate-y-2 active:shadow-none">
                        {isGeneratingHw ? <Loader2 className="animate-spin" size={28} /> : <Sparkles size={28} />} ÜRET
                      </button>
                    </div>
                    <div className="grid md:grid-cols-2 gap-10">
                      <div>
